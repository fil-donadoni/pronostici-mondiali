...
export function qualifyThirds(
  ...
  return rankThirds(standings).slice(0, 8).map((t) => t.row.teamId);
}
...
function winnerOfPrediction(
  ...
  if (p.penaltyWinner === "home") return { winnerId: homeId, loserId: awayId };
  if (p.penaltyWinner === "away") return { winnerId: awayId, loserId: homeId };
