...
function slotLabel(s: Slot): string {
  switch (s.kind) {
    case "winner-group": return `1${s.group}`;
    case "runner-group": return `2${s.group}`;
    case "third": return "3RD";
    case "winner-of": return `W:${s.matchId}`;
    case "loser-of": return `L:${s.matchId}`;
  }
}
...
async function seed() {
  ...
  if (useApi) {
    ...
    try {
      ...
      console.log(`  ✓ ${teams.length} squadre reali da football-data.org`);
    } catch (e) {
      console.warn("  ! fetch reale fallito, uso il fallback:", (e as Error).message);
      teams = FALLBACK_TEAMS;
    }
  } else {
    console.log("→ Nessuna API key: uso dataset di fallback (48 nazionali).");
  }
  ...
  await db.insert(match).values(
    KNOCKOUT_MATCHES.map((m) => ({
      ...
      kickoff: new Date(koBase + (m.matchNumber - 72) * 24 * 60 * 60 * 1000),
      homeTeamId: null,
      awayTeamId: null,
