  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isSignup) {
        ...
        if (error) throw new Error(error.message ?? "Errore registrazione");
        ...
        <CardFooter className="flex-col gap-3 mt-6">
          ...
          <p className="text-sm text-muted-foreground">
            {isSignup ? (
              <>
                Hai già un account?{" "}
                <Link href="/login" className="underline">Accedi</Link>
              </>
            ) : (
              <>
                Non hai un account?{" "}
                <Link href="/signup" className="underline">Registrati</Link>
              </>
            )}
          </p>
        </CardFooter>
