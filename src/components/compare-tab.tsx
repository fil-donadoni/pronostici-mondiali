    ...
    return (
      <Card>
        <CardContent className="py-10 text-center text-muted-foreground">
          ...
          <span className="font-medium text-foreground">Sync risultati</span> in
          alto per scaricarli e confrontarli con i tuoi pronostici.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard label="Risultato esatto" value={summary.exact} tone="ok" />
        <StatCard label="Esito giusto" value={summary.correctOutcome} tone="warn" />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Differenze nei gironi</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-1">
            {diffs.map((d) => {
              const m = matchById.get(d.matchId);
              const homeName = m?.homeTeamId ? name(m.homeTeamId) : "?";
              const awayName = m?.awayTeamId ? name(m.awayTeamId) : "?";
              ...
              return (
                ...
                >
                  ...
                  <span className="tabular-nums font-medium w-20 text-center">
                    <span className={tone === "bad" ? "text-muted-foreground line-through" : ""}>
                      {d.predicted.home}-{d.predicted.away}
                    </span>
                    {!d.exactMatch && (
            ...
            <div key={r.stage}>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-medium text-sm">
                  {STAGE_LABEL[r.stage as keyof typeof STAGE_LABEL] ?? r.stage}
                </span>
                {!r.hasReal && (
              ...
              <div className="flex flex-wrap gap-1">
                {r.predicted.map((t) => {
                  const matched = r.hasReal && !r.onlyPredicted.includes(t);
                  return (
                    <Badge
                      key={t}
                      variant={
                        !r.hasReal ? "secondary" : matched ? "default" : "destructive"
                      }
                      className="text-[11px]"
                    >
                      {name(t)}
              ...
              {r.onlyReal.length > 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  Reali non previste: {r.onlyReal.map(name).join(", ")}
                </p>
              )}
            </div>
          ))}
